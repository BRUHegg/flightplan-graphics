/*
   ...
   https://creativecommons.org/licenses/by-nc-sa/4.0/
        ...
        This header file contains definitions of utility functions for cairo
   library. Author: discord/bruh4096#4512(Tim G.)
*/
...
constexpr geom::vect3_t BLACK = {0, 0, 0};
constexpr geom::vect3_t DARK_BLUE = {0.01, 0.05, 0.15};
constexpr geom::vect3_t MAGENTA = {1, 0.451, 1};
constexpr geom::vect3_t ND_CYAN = {0.051, 0.757, 0.951};
constexpr geom::vect3_t ND_LT_GREY = {0.657, 0.657, 0.657};
...
inline void prepare_cairo_context(cairo_t* cr, geom::vect3_t color,
                                  int line_width) {
...
inline void draw_rect(cairo_t* cr, geom::vect2_t pos, geom::vect2_t sz,
  ...
  if (line_width > 0) cairo_stroke(cr);
...
inline void draw_tri(cairo_t* cr, geom::vect2_t v1, geom::vect2_t v2,
                     geom::vect2_t v3, geom::vect3_t color,
                     double line_width = -1) {
  prepare_cairo_context(cr, color, line_width);
...
inline void draw_arc(cairo_t* cr, geom::vect2_t pos, double radius,
...
inline void draw_rotated_image(cairo_t* cr, cairo_surface_t* surf,
                               geom::vect2_t pos, geom::vect2_t scale,
                               double rot_ang_rad) {
  if (scale.x == 0 || scale.y == 0) return;
