...
FPLSys::FPLSys(std::shared_ptr<libnav::ArptDB> arpt_db,
  ...
  for (size_t i = 0; i < N_FPL_SYS_RTES; i++) {
    std::shared_ptr<FplnInt> tmp = std::make_shared<FplnInt>(
        arpt_db_ptr_, navaid_db_ptr_, awy_db_ptr_, cifp_dir_path);
...
bool FPLSys::get_exec() { return m_exec_st; }

size_t FPLSys::get_act_idx() { return act_rte_idx; }
...
size_t FPLSys::get_nd_seg(nd_leg_data_t* out, size_t n_max, size_t idx) {
  ...
  if (fpl_datas[idx].leg_list.size() == 0) return 0;
  size_t n_written = 0;
  size_t i_start = 1;
  ...
  for (size_t i = i_start; i < fpl_datas[idx].leg_list.size() - 1; i++) {
    ...
    if (fpl_datas[idx].leg_list[i].data.is_discon) continue;
...
int FPLSys::get_act_leg_idx(size_t idx) {
  ...
  if (fpl_datas[idx].act_leg_idx == -1) return -1;
  return 1;
}
...
bool FPLSys::get_ctr(geo::point* out, size_t sd_idx) {
  ...
  if (sd_idx) curr_idx = fpl_datas[idx].fo_ctr_idx;
...
void FPLSys::step_ctr(bool bwd, size_t sd_idx) {
  size_t idx = cdu_sel_fpl[sd_idx];
  if (!fpl_datas[idx].leg_list.size()) return;
...
void FPLSys::set_flt_nbr(std::string str) { flt_nbr = str; }

std::string FPLSys::get_flt_nbr() { return flt_nbr; }
...
RTECopySts FPLSys::act_can_copy() {
  if (act_rte_idx != N_FPL_SYS_RTES && !m_exec_st) {
    ...
    if (id1 != copy_ids[0] || id2 != copy_ids[1]) return RTECopySts::READY;
...
void FPLSys::copy_act() {
  if (act_rte_idx != N_FPL_SYS_RTES && !m_exec_st) {
    ...
    if (act_rte_idx == RTE2_IDX) tgt_idx = RTE1_IDX;
...
void FPLSys::update() {
  ...
  if (act_rte_idx < N_FPL_SYS_RTES && !fpl_vec_[act_rte_idx]->can_activate() &&
      !m_exec_st) {
    act_rte_idx = N_FPL_SYS_RTES;
    act_id = -1;
  }
...
void FPLSys::update_seg_list(size_t idx) {
  ...
  fpl_datas[idx].seg_list_id =
      fpl_vec_[idx]->get_sl_seg(0, sz, &fpl_datas[idx].seg_list);
}
...
void FPLSys::update_leg_list(size_t idx) {
  ...
  fpl_datas[idx].leg_list_id = fpl_vec_[idx]->get_ll_seg(
      0, sz, &fpl_datas[idx].leg_list, &fpl_datas[idx].act_leg_idx);
