...
namespace geom {
enum class JointType { CIRC, CIRC_CIRC, LINE };
...
struct vect3_t {
  ...
  double absval() { return sqrt(x * x + y * y + z * z); }

  vect3_t scmul(double num) { return {x * num, y * num, z * num}; }
...
inline double get_vec_to_line(vect2_t px, vect2_t pl1, vect2_t pl2,
                              vect2_t* out) {
  ...
  if (nml.dot_prod(a) < 0) nml = nml.scmul(-1);
...
inline double get_vect_circ_isect_dist(vect2_t ps, vect2_t po, vect2_t a,
                                       double c_r) {
  vect2_t b = po - ps;

  double abs_b = b.absval();
  if (abs_b == c_r) return 0;
...
inline double get_turn_rad(double ang_start_rad, double ang_end_rad,
                           bool left_turn) {
...
inline arc_t get_circ_from_pts(vect2_t pos, vect2_t start, vect2_t end,
  ...
  if (ang_start_rad < 0) ang_start_rad += 2 * M_PI;
  if (ang_end_rad < 0) ang_end_rad += 2 * M_PI;
...
inline line_joint_t get_line_joint(vect2_t pq, vect2_t ps, vect2_t pa,
                                   vect2_t pb, double radius,
                                   double str_join_deg = 5) {
  line_joint_t out = {};
  ...
  if ((sa.x == 0 && sa.y == 0) ||
      (abs(c) < sin(str_join_deg * DEG_TO_RAD) && qs.dot_prod(sb) >= 0 &&
  ...
  if (nml_offs <= 2 * radius) {
    ...
    out.arc2 =
        get_circ_from_pts(po2, po2 + ctr_vec.scmul(-1), ln_start, !left_turn);
